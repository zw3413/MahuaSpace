				...
				<div class="main-col <?php echo esc_attr($is_text_chapter_right_sidebar ? "col-md-8" : "col-md-12"); ?> col-sm-12 sidebar-hidden">
					...
					if ($madara_show_chapter_heading == 'on') { ?>
						<h1 id="chapter-heading"><?php
													...
													echo esc_html($manga->post_title); ?> - <?php echo esc_html($reading_chapter['chapter_name']); ?></h1>
					<?php }  ?>
...
<script>
	...
	jQuery(function() {
		...
		if ($('.btn-active-modal').length > 0) {
			...
			$('#form-login').unbind('click'); //取消点击取消功能
			// $('.to-register').click(
			// 	function(){
			// 		jumpToRegister()
			// 	}
			...
			// 	setTimeout(function() {
			// 		jQuery('#form-login').unbind('click');
			// 		jQuery('#sign-up a').eq(3).hide()
			// 	}, 1000);
			// 	jQuery('#login a').eq(2).hide();
